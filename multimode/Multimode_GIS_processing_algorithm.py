...
#from .modules.apikey.key import keytpvp, keytpvelo, keytpmarche, keytrafic, keytptransit, token, tokenpubtransit,
from .modules.apikey.key import Herekey
...
class Multimode_GIS_processingAlgorithm(QgsProcessingAlgorithm):
    ...
    TPMS_MAX_MARCHE = 'TPMS_MAX_MARCHE'
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterField(
                ...
                '',
                parentLayerParameterName=self.INPUT,  # Lie le champ à la couche d'entrée
        ...
        self.addParameter(
            QgsProcessingParameterString(
                self.TPMS_MAX_MARCHE,
                self.tr("Temps max marche à pied (en mètres)"),
                defaultValue="1000"
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        tps_marche_max = self.parameterAsString(parameters,self.TPMS_MAX_MARCHE, context)
        ...
        new_fields = [
            ...
            QgsField("Heure de départ TC", QVariant.DateTime),
            QgsField("Heure d'arrivée TC", QVariant.DateTime),
        ...
        for current, feature in enumerate(features):
            ...
            # Récupère les attributs existants
            ...
            if geometry and geometry.type() == QgsWkbTypes.PointGeometry:
                point = geometry.asPoint()
                s_olng, s_olat = point.x(), point.y()
            else:
                feedback.pushInfo(f"Feature {feature.id()} n'a pas de géométrie valide. Ignoré.")
                continue
            #Récupérer laes valeur xy de destination
            s_dlng = feature[self.FIELD_X_DEST]  
            s_dlat = feature[self.FIELD_Y_DEST]
    ...
    def groupId(self):
        ...
        formatting characters.
        """
        return 'multimode_group'

    def tr(self, string):
        return QCoreApplication.translate('Multimode_GIS_processingAlgorithm', string)
